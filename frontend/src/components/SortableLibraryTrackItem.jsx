...
import { Play, Disc3, Clock, Trash2, Heart, Plus, GripVertical } from 'lucide-react';

const SortableLibraryTrackItem = ({ track, index, onPlay, onDelete, onAddToPlaylist, onToggleFavourite, isFavourite, formatDuration }) => {
  const {
    attributes,
    listeners,
    setNodeRef,
    ...
    >
      ...
      <div className="flex-1 flex items-center gap-4 min-w-0">
        <div className="relative w-12 h-12 rounded-md bg-gradient-to-br from-primary-600/50 to-primary-900/50 flex-shrink-0 flex items-center justify-center overflow-hidden">
          <Disc3 className="w-6 h-6 text-slate-800/50 dark:text-slate-100/50" />
        </div>
      ...
      <div className="w-32 hidden xl:block text-sm text-slate-500 dark:text-slate-400">
      ...
      <div className="w-32 flex items-center justify-end gap-2 px-2">
        <button 
          onClick={(e) => { e.stopPropagation(); onAddToPlaylist(track, e); }}
        ...
        <button 
          onClick={(e) => { e.stopPropagation(); onToggleFavourite(e, track.id); }}
...
export default SortableLibraryTrackItem;
