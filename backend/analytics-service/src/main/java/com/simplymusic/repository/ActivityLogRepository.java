...
public interface ActivityLogRepository extends MongoRepository<ActivityLog, String> {
    ...
    @Aggregation(pipeline = {
            "{ $match: { eventType: 'TRACK_PLAYED', userId: { $in: ?0 }, trackId: { $in: ?1 } } }",
            "{ $group: { _id: '$trackId', playCount: { $sum: 1 }, lastPlayed: { $max: '$timestamp' } } }",
            "{ $project: { trackId: '$_id', playCount: 1, lastPlayed: 1, _id: 0 } }",
            "{ $sort: { lastPlayed: -1 } }"
    })
    List<TrackHistory> getPlaybackHistory(List<String> userIds, List<String> activeTrackIds);

    @Aggregation(pipeline = {
            "{ $match: { eventType: 'TRACK_PLAYED', userId: { $in: ?0 }, trackId: { $in: ?1 } } }",
            "{ $group: { _id: '$trackId', playCount: { $sum: 1 }, lastPlayed: { $max: '$timestamp' } } }",
            "{ $project: { trackId: '$_id', playCount: 1, lastPlayed: 1, _id: 0 } }",
            "{ $sort: { playCount: -1, lastPlayed: -1, trackId: 1 } }",
            "{ $limit: 10 }"
    })
    List<TrackHistory> getTopTracks(List<String> userIds, List<String> activeTrackIds);
}
